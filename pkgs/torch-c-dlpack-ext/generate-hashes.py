"""
torch-c-dlpack-ext generate-hashes configuration module.

Imported by the shared entry point ``generate-hashes/main.py``.
Do NOT add a main() here.

torch-c-dlpack-ext wheels are distributed via PyPI (originType "pypi"),
one manylinux wheel per CPython version (no abi3, no per-torch builds).

Invocation (from project root):
  nix run .#default.torch-c-dlpack-ext.gen-hashes
  nix run .#default.torch-c-dlpack-ext.gen-hashes -- --tag 0.1.5
...
# Source:  https://pypi.org/project/torch-c-dlpack-ext/
# To regenerate: nix run .#default.torch-c-dlpack-ext.gen-hashes [-- --tag {version}]
#
# torch-c-dlpack-ext {version} binary-wheel hashes.
# Wheels are torch-agnostic (one wheel per CPython version).
#
...
def run() -> None:
    p = argparse.ArgumentParser(
        description="Generate binary-hashes for torch-c-dlpack-ext (PyPI wheels)."
    )
    ...
    if args.tag:
        ...
        versions = fetch_pypi_versions(PROJECT)
        print(f"{len(versions)} {PROJECT} version(s): " + ", ".join(versions))
    ...
    for version in versions:
        ...
        organized = collect_pyver_wheels(PROJECT, version)
        if not organized:
