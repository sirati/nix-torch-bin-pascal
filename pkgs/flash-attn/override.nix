...
let
  ...
  _assertLinux = if os == "linux" then true
    else throw "flash-attn pre-built wheels are only available for Linux (got: ${os})";
  ...
  # Extract major.minor from the torch derivation's version, e.g. "2.9.1" -> "2.9"
  ...
  torchCompat =
    let
      ...
      findFirst = cs:
        if cs == []
        then throw (
          "flash-attn ${flashAttnVersion} (${cudaVersion}): no compatible pre-built wheel found for "
...
pkgs.python3Packages.buildPythonPackage {
  ...
  # Runtime Python dependencies (mirrors upstream nixpkgs flash-attn)
  dependencies = [
