...
"""
Generate mamba-ssm binary-hashes and source-hashes from GitHub releases.
...
Run from the project root or from within pkgs/mamba-ssm/:
  nix-shell pkgs/mamba-ssm/generate-hashes.py
  nix-shell pkgs/mamba-ssm/generate-hashes.py -- --tag v2.3.0
  nix-shell pkgs/mamba-ssm/generate-hashes.py -- --skip-source
  nix-shell pkgs/mamba-ssm/generate-hashes.py -- --source-only
...
GITHUB_REPO = "state-spaces/mamba"
...
# Matches mamba_ssm wheel filenames released on GitHub, e.g.:
#   mamba_ssm-2.3.0+cu12torch2.6cxx11abiTRUE-cp313-cp313-linux_x86_64.whl
#   mamba_ssm-2.3.0+cu12torch2.5cxx11abiTRUE-cp312-cp312-linux_x86_64.whl
#   mamba_ssm-2.3.0+cu11torch2.4cxx11abiFALSE-cp310-cp310-linux_x86_64.whl
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(
        description="Generate mamba-ssm binary-hashes and source-hashes.",
    )
    add_common_args(p)
    args = p.parse_args()
    ...
    run_all_hashes(
        ...
        _HERE, args,
        with_submodules=False,  # mamba has no git submodules
    )
